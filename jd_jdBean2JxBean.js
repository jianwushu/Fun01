...
!(async () => {
    if (!cookiesArr[0]) {
        $.msg($.name, '【提示】请先获取京东账号');
        return;
    }
    for (let i = 0; i < cookiesArr.length; i++) {
        JXUA = USER_AGENTS.JX_USER_AGENT
    ...
    $.get(n, (n, o, a) => {
      try {
        if (n) $.logErr(n);
          ...
          0 === a["retcode"] &&
            a.data &&
            a.data.hasOwnProperty("userInfo") &&
            ($.nickName = a.data.userInfo.baseInfo.nickname),
            0 === a["retcode"] &&
              a.data &&
              a.data["assetInfo"] &&
              ($.beanCount = a.data && a.data["assetInfo"]["beanNum"]);
