...
function saveTaskRecord(ck, taskId, businessId, taskType) {
  let opt = {
    ...
    headers: {
      ...
      "User-Agent":
        "Mozilla/5.0 (iPhone; CPU iPhone OS 15_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 JDFanli/2.0.20 jd.fanli/2.0.20",
      // "Sec-Fetch-Mode": "cors",
      ...
      try {
        if (err) {
          ...
          if (data) {
            data = JSON.parse(data);
            if (data.content) {
              if ((data.content.status = 1)) $.count.finishCount += 1;
              console.log("浏览结果", data.content.msg);
            } else console.log("结果", data);
          } else {
            $.log("京东返回了空数据");
          }
        }
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            console.log(
              "已完成次数：" +
                data.content.finishCount +
                "  总任务次数：" +
                data.content.maxTaskCount
            );
            $.count = data.content;
          }
        }
